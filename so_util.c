...
uintptr_t trampoline_start = 0;
uintptr_t trampoline_end = 0;

void so_write_memory(uintptr_t addr, void* src, size_t size)
{
  kuKernelCpuUnrestrictedMemcpy((void*)addr, src, size);
}

void so_write_hooklib(uintptr_t addr, uintptr_t func)
{
  char code[16];
  sceClibMemcpy(code, "\x01\xB4\x01\xB4\x01\x48\x01\x90\x01\xBD\x00\xBF\x00\x00\x00\x00", 16);
  *(uint32_t*)&code[12] = (func | 1);
  so_write_memory(addr, (uintptr_t)code, 16);
}

void so_asm_jmp(uintptr_t func, uintptr_t addr)
{
	uint32_t code = ((addr-func-4) >> 12) & 0x7FF | 0xF000 | ((((addr-func-4) >> 1) & 0x7FF | 0xB800) << 16);
  so_write_memory(func, (uintptr_t)&code, 4);
}

void so_init_trampolines(uintptr_t addr, uintptr_t size)
{
  trampoline_start = addr;
	trampoline_end = trampoline_start + size;
}

void so_hook_setup(uintptr_t addr, uintptr_t func, uintptr_t* orig)
{
  so_write_memory(trampoline_start, addr, 4);
  so_write_hooklib(trampoline_start + 4, addr + 4);
  *orig = trampoline_start + 1;
  trampoline_start += 20;

  so_asm_jmp(addr, trampoline_start);
  so_write_hooklib(trampoline_start, func);
  trampoline_start += 20;
}
